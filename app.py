...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'stocksmart-secret'
...
def detect_anomalies(user_id):
    ...
    daily = {}
    for s in sales: daily[s.created_at.date()] = daily.get(s.created_at.date(), 0) + s.total_amount
    arr = [daily.get(end - timedelta(i), 0) for i in range(30)]
    mean, std = np.mean(arr), np.std(arr)
    if std == 0: return
    today_sales = daily.get(end, 0)
    ...
    if abs(z) > 1.5:
        ...
        if not AnomalyLog.query.filter_by(user_id=user_id, date=end).first():
            db.session.add(AnomalyLog(user_id=user_id, date=end, anomaly_type=typ, severity=abs(z),
                                      expected_sales=mean, actual_sales=today_sales,
                                      notes=f"{typ} detected. Expected ~{mean:.0f} got {today_sales:.0f}"))
            db.session.commit()

def train_forecast(user_id):
    sales = Sale.query.filter_by(user_id=user_id).all()
    if len(sales) < 7: return None, None
    data = {}
    for s in sales: data[s.created_at.strftime('%Y-%m-%d')] = data.get(s.created_at.strftime('%Y-%m-%d'), 0) + s.total_amount
...
def login():
    ...
    if request.method == 'POST':
        ...
        if u and u.check_password(request.form['password']):
            ...
            flash(f'Welcome {u.shop_name}!', 'success')
            return redirect(url_for('dashboard'))
...
def dashboard():
    ...
    low_count = sum(1 for p in products if p.quantity <= p.low_stock_threshold)
    tax_due, days = None, None
    if current_user.tin and current_user.is_vat_registered:
        td = datetime.today()
        if td.day < 20: tax_due = datetime(td.year, td.month, 20)
        else: tax_due = datetime((td.replace(day=1)+timedelta(32)).year, (td.replace(day=1)+timedelta(32)).month, 20)
        days = (tax_due - td).days
    detect_anomalies(current_user.id)
    anomaly = AnomalyLog.query.filter_by(user_id=current_user.id, date=today).first()
    return render_template('dashboard.html', total_sales_today=total_sales_today,
                         total_profit_today=total_profit_today, total_stock_value_cost=stock_cost,
                         low_stock_count=low_count, tax_due_date=tax_due, days_left=days, today_anomaly=anomaly)

@app.route('/products')
@login_required
def products():
    return render_template('products.html', products=Product.query.filter_by(user_id=current_user.id).order_by(Product.name).all())

@app.route('/add_product', methods=['GET','POST'])
@login_required
def add_product():
    if request.method == 'POST':
        p = Product(user_id=current_user.id, name=request.form['name'],
                    cost_price=float(request.form['cost_price']), selling_price=float(request.form['selling_price']),
                    quantity=float(request.form['quantity']), low_stock_threshold=float(request.form.get('low_stock_threshold',5)),
...
def adjust_stock(id):
    ...
    if p.user_id != current_user.id: flash('Access denied','danger')
        ...
        p.quantity += qty
        db.session.add(StockHistory(user_id=current_user.id, product_id=p.id,
                     change_type='add' if qty>0 else 'remove', quantity=abs(qty), note=request.form.get('note','')))
...
def sales_report():
    ...
    today = date.today()
    if filter_type == 'today': start = end = today
    elif filter_type == 'week': start = today - timedelta(days=today.weekday()); end = today
    elif filter_type == 'month': start = today.replace(day=1); end = today
...
def tax_reminder():
    ...
    if user.is_vat_registered and user.tin:
        ...
        vat_due = sum(s.total_amount for s in sales) * 0.18
    td = datetime.today()
...
@login_required
def ai_forecast():
    model, last_date = train_forecast(current_user.id)
    if model is None: flash('Need 7 days of sales data','warning'); return redirect(url_for('dashboard'))
    ...
    advice = [f"📈 Avg predicted daily: TZS {sum(preds)/7:,.0f}"]
    ...
    else: advice.append("✅ Sales stable – maintain levels")
...
def init_db():
    ...
    if not User.query.first():
        admin = User(shop_name='Admin Shop', phone='0712345678', email='admin@stocksmart.com',
                     tin='123456789', is_vat_registered=True, is_admin=True)
